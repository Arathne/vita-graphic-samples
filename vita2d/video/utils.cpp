/* code belongs to SonicMastr
 * https://github.com/SonicMastr/Vita-Media-Player/blob/main/src/avplayer/avplayerUtils.c
*/
...
void *avTextureAlloc(SceKernelMemBlockType type, unsigned int size, unsigned int alignment, unsigned int attribs, SceUID *uid)
{
	...
	ret = sceGxmMapMemory(mem, size, (SceGxmMemoryAttribFlags) attribs);
...
void avTextureFree(void *mem, SceUID memUid)
{
	...
	if (memUid == SCE_KERNEL_ERROR_INVALID_UID) {
...
{
	avTextureFree(pMemory, SCE_KERNEL_ERROR_INVALID_UID);
};
